...
public:
	...
	// Returns false if all points are outside of the same place.
	bool isConvexHullSeen(SafePointer<const FVector3D> cameraSpacePoints, int32_t pointCount) const {
...
public:
	...
	// Returns false iff the model inside of the bound can clearly not be visible
	//   by having all corners outside of the same side in the camera's culling frustum.
	bool isBoxSeen(const FVector3D& minModelSpaceBound, const FVector3D& maxModelSpaceBound, const Transform3D &modelToWorld) const {
