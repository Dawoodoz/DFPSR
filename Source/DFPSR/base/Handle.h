...
namespace dsr {
	...
	public:
		...
		inline uintptr_t getElementCount() const {
			...
			return heap_getSize(this->data) / sizeof(T);
		}
		...
		SafePointer<V> getSafe(const char * name) const {
			if (this->data == nullptr) {
				...
				#ifdef SAFE_POINTER_CHECKS
					AllocationHeader *header = heap_getHeader(this->data);
					return SafePointer<V>(header, this->allocationIdentity, name, (V*)this->data, this->getSize(), (V*)this->data);
				#else
					return SafePointer<V>(name, (V*)this->data, buffer_getSize(buffer), (V*)this->data);
				#endif
			}
		}
		...
		if (result.isNull()) {
			...
			if (!std::is_trivially_destructible<T>::value) {
				...
				heap_setAllocationDestructor(result.getUnsafe(), HeapDestructor([](void *toDestroy, void *externalResource) {
					...
					uintptr_t elementCount = heap_getSize(toDestroy) / sizeof(T);
