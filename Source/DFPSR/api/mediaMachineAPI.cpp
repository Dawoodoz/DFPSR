...
MediaMachine machine_create(const ReadableString& code) {
	Handle<PlanarMemory> memory = handle_create<MediaMemory>();
	...
	return MediaMachine(handle_create<VirtualMachine>(code, memory, mediaMachineInstructions, mediaMachineInstructionCount, mediaMachineTypes, mediaMachineTypeCount));
}
