...
* **Pixel exact 2D** Instead of making strange workarounds over polygons, a software renderer lets you work with whole pixels to begin with.
...
* **3D rendering** Roughly equivalent to Direct3D 7 with bi-linear texture sampling, mipmapping, lightmaps and alpha filtering when used out of the box, but can be modified to be more like Direct3D 9 if you apply shading to textures (can use SIMD with multi-threading and be scheduled based on viewing distance).
...
* **Callbacks** TemporaryCallback guarantees that no heap allocation is made for copying the lambda's closure, by only allowing it to be passed by reference to functions using it directly.
